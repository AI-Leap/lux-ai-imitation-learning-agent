...
class GameMap:
    ...
    def get_adjacent_cells_by_pos(self, pos):
        """
        :param cell:
        :return:
        """
        ...
        # NORTH
        if pos.y > 0:
            cells.append(self.get_cell(pos.x, pos.y - 1))

        # EAST
        if pos.x < self.width - 1:
            cells.append(self.get_cell(pos.x + 1, pos.y))
    ...
    def get_adjacent_cells_with_corner_by_pos(self, pos):
        cell = self.get_cell(pos.x, pos.y)

        cells = self.get_adjacent_cells_by_pos(pos)
